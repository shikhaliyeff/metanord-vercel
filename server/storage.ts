...
export interface IStorage {
  ...
  updateDocumentCategory(id: number, data: Partial<InsertDocumentCategory>): Promise<DocumentCategory | undefined>;
  deleteDocumentCategory(id: number): Promise<boolean>;
  
  // Document management
  getAllDocuments(language?: string, documentType?: string, productCategory?: string, categoryId?: number): Promise<Document[]>;
...
export class MemStorage implements IStorage {
  ...
  documentCategoryCurrentId: number;
  ...
  async updateDocumentCategory(id: number, data: Partial<InsertDocumentCategory>): Promise<DocumentCategory | undefined> {
    ...
    if (data.name && !data.slug) {
      updatedCategory.slug = this.slugify(data.name);
    }
  ...
  async deleteDocumentCategory(id: number): Promise<boolean> {
    const exists = this.documentCategories.has(id);
    if (!exists) return false;
    
    // Check if any documents use this category before deleting
    const documents = Array.from(this.documents.values())
      .filter(doc => doc.documentCategoryId === id);
    
    if (documents.length > 0) {
      // Update documents to remove the category reference
      for (const doc of documents) {
        const updatedDoc = { ...doc, documentCategoryId: null };
  ...
  // Utility function to create slugs from names
  ...
  async getAllDocuments(language?: string, documentType?: string, productCategory?: string, categoryId?: number): Promise<Document[]> {
    const documents = Array.from(this.documents.values());
    ...
    return documents.filter(doc => {
      ...
      if (categoryId !== undefined && doc.documentCategoryId !== categoryId) {
        match = false;
      }
