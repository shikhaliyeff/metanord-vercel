...
import { 
  ...
  translateCategory, 
  preloadProductData 
} from "@/lib/unified-product-loader";
import { preloadImages, optimizeContentVisibility } from "@/lib/asset-optimization";
...
// Global product image map for faster access
...
// Core product IDs for each category
...
export default function EnhancedProductsPage() {
  ...
  // Enhanced optimization for performance
  useEffect(() => {
    console.log("Starting filtering with category: all, language: " + i18n.language);
    console.log("Total available products: " + allProductIds.length);
    console.log("Available product types:", productCategories);
    
    // Pre-translate categories for performance optimization
    productCategories.forEach(cat => {
      translateCategory(cat, i18n.language);
    });
    
    // Preload critical product data to improve performance
    preloadProductData(allProductIds.slice(0, 4), i18n.language);
    
    // Preload critical product images
    const criticalImages = [
      productImagesMap['standard-profiles'],
      productImagesMap['hdpe-pipes'],
      productImagesMap['manhole-covers']
    ];
    preloadImages(criticalImages, true);
    
    // Apply content visibility optimization for better rendering performance
    if (pageRef.current) {
      optimizeContentVisibility(pageRef.current);
    }
    ...
    // Dynamically create schema.org script tag
